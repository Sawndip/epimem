...
int main(int argc, char* argv[]){
  ...
  for(numberofpatterns = 0; numberofpatterns <= largest; numberofpatterns++){  
    ...
    int p1 = 0;
    std::cout<<"pattern to remember:"<<numberofpatterns<<std::endl;
    ...
    */
    ...
    for(b = 0; b < patterns.size(); b++){
      ...
      for(i = 0; i < model->length; i++)model->localfield[i] = 0.5*(double)(2*init[i] -1);
      int notzero = 0;
